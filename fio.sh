...
run_io_benchmk()
{
	local cur_out_dir=$1
	local loop_cnt=$2
	if [ ! -e $cur_out_dir/io_bench.csv ]; then
		echo -e "loop R-iops R-bw W-iops W-bw RR-iops RR-bw RW-iops RW-bw" \
				> $cur_out_dir/io_bench.csv
	fi
	...
	if [ $? -eq 0 ]; then
		...
		echo "$loop_cnt $fio_save" >> $cur_out_dir/io_bench.csv
	else
		echo -n ".."
		echo -e "[${RED}error${END}]"
	fi
